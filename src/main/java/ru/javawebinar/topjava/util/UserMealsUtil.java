...
public class UserMealsUtil {
    public static void main(String[] args) {
        ...
        List<UserMealWithExceed> filteredWithExceededStream = getFilteredWithExceededStream(mealList, LocalTime.of(7, 0), LocalTime.of(12, 0), 2000);
                filteredWithExceededStream.forEach(System.out::println);
//        .toLocalDate();
//        .toLocalTime();
    }
    ...
    public static List<UserMealWithExceed>  getFilteredWithExceeded(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        ...
        for (UserMeal Umeal: mealList){
            LocalDate mealDate = Umeal.getDateTime().toLocalDate();
            res.put( mealDate, res.getOrDefault(mealDate, 0) + Umeal.getCalories());
        }

        List<UserMealWithExceed> list = new ArrayList<>();
        for(UserMeal Umeal: mealList){
            if(TimeUtil.isBetween(Umeal.getDateTime().toLocalTime(),startTime,endTime)) {
                list.add(new UserMealWithExceed(Umeal.getDateTime(), Umeal.getDescription(), Umeal.getCalories(),
                        res.get(Umeal.getDateTime().toLocalDate()) > caloriesPerDay));
            }
        }
    ...
    public static List<UserMealWithExceed> getFilteredWithExceededStream(
